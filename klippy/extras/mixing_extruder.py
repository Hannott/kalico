...
def normalize_mixing(mixing):
    normalized = tuple(
        0 if m < EXTRUSION_FACTOR_THRESHOLD else m for m in mixing
    )
    s = sum(normalized)
...
def mixing_preset(idx, steppers_count):
    if idx == 0:
        return [1.0 / steppers_count for p in range(steppers_count)]

    idx = idx - 1
    if idx < steppers_count:
        return [1.0 if p == idx else 0.0 for p in range(steppers_count)]

    idx = idx - steppers_count
    if idx < steppers_count:
        return [
            0.0 if p == idx else 1.0 / (steppers_count - 1)
    ...
    if steppers_count == 3:
        if idx < 2 * steppers_count:
            return [
                [
                    0.0 if p == x else (1 + (((p - x) % 3 + y) % 2)) / 3.0
                    for p in range(steppers_count)
                ]
        ...
        if idx < (steppers_count * (steppers_count - 1) / 2):
            return [
                [
                    0.0 if p == x or p == y else 1.0 / (steppers_count - 2)
                    for p in range(steppers_count)
                ]
                for x in range(steppers_count)
                for y in range(x + 1, steppers_count)
            ][idx]
...
class MixingExtruder:
    def __init__(self, config):
        ...
        self.stepper_names = tuple(
            e.strip() for e in config.get("steppers", "").split(",") if len(e)
        )
        if not len(self.stepper_names):
            raise self.printer.config_error("No steppers configured for mixing")

        self._extruder_steppers = []
        self._rotation_distances = ()
        self._trapqs = ()
        self.extruder_name = config.get("extruder_name", self.stepper_names[0])
        self._mixing = self._active_mixing = mixing_preset(
            0, len(self.stepper_names)
        )
        self._enabled = False
    ...
    def _handle_connect(self):
        ...
        self._trapqs = tuple(
            stepper.get_trapq() for _, stepper in self._extruder_steppers
        )

    def get_status(self, eventtime):
        return dict(
            mixing=",".join("%0.1f" % (m * 100.0) for m in self._active_mixing),
            ticks=",".join(
                "%d" % (stepper.get_mcu_position())
                for _, stepper in self._extruder_steppers
            ),
        )
    ...
    def _apply_mixing(self, mixing):
        ...
        for i, (extruder_stepper, stepper) in enumerate(
            self._extruder_steppers
        ):
    ...
    def cmd_SET_MIXING_EXTRUDER(self, gcmd):
        if (
            gcmd.get("FACTORS", None)
            or gcmd.get("PRESET", None)
            or gcmd.get("ENABLE", None)
        ):
            factors = gcmd.get("FACTORS", None)
            preset = gcmd.get_int("PRESET", 0, minval=0)
            enable = gcmd.get_int("ENABLE", 1, minval=0, maxval=1)
            ratios = [
                1 if p == 0 else 0 for p in range(len(self.stepper_names))
            ]

            if factors:
                try:
                    factors = [
                        float(s) for s in factors.replace(",", ":").split(":")
                    ]
            ...
            gcmd.respond_info(
                mix
                + ",".join("%0.1f%%" % (m * 100.0) for m in self._active_mixing)
            )
            gcmd.respond_info(
                mix
                + ",".join(
...
class GradientMixingExtruder(MixingExtruder):
    ...
    def _get_gradient(self, pos):
        ...
        for heights, refs in self._gradients:
            ...
            if self._gradient_method == "linear":
                ...
                return list(
                    ((1.0 - w) * s + w * e) for s, e in zip(start_mix, end_mix)
                )

            if self._gradient_method == "spherical":
                dist = math.sqrt(
                    sum(
                        (x - self._gradient_vector[i]) ** 2
    ...
    def cmd_SET_MIXING_EXTRUDER_GRADIENT(self, gcmd):
        if gcmd.get("START_FACTORS", ""):
            ...
            start_height = gcmd.get_float("START_HEIGHT", minval=0.0)
            end_height = gcmd.get_float("END_HEIGHT", minval=0.0)
            ...
            try:
                ...
                self._gradient_vector = tuple(
                    float(vector[i]) for i in range(3)
                )
            except Exception:
                raise gcmd.error("Could not configure gradient: invalid vector")
            ...
            for i, gradient in enumerate(self._gradients):
                ...
                gcmd.respond_info(
                    ...
                    + "mixing=%s-%s"
                    % tuple(
                        "/".join("%.1f" % (x) for x in m) for m in gradient[1]
                    )
                )
                gcmd.respond_info(grad + "method=%s" % (self._gradient_method))
                gcmd.respond_info(
                    grad
                    + "vector=%s"
                    % ("/".join("%.1f" % (x) for x in self._gradient_vector))
                )
                gcmd.respond_info(

...
class BMI160:
    def __init__(self, config):
        ...
        if config.get("cs_pin", None) is not None:
            ...
            self.bus = bus.MCU_I2C_from_config(
                config, default_addr=BMI_I2C_ADDR, default_speed=400000
            )
            self.bus_type = "i2c"
        ...
        mcu.add_config_cmd(
            "query_bmi160 oid=%d rest_ticks=0" % (oid,), on_restart=True
        )
        ...
        self.batch_bulk.add_mux_endpoint(
            "bmi160/dump_bmi160", "sensor", self.name, {"header": hdr}
        )
    ...
    def _build_config(self):
        ...
        self.ffreader.setup_query_command(
            "query_bmi160_status oid=%c", oid=self.oid, cq=cmdqueue
        )

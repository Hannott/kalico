...
def assert_is_int32(value, frac_bits):
    if value > MAX_INT32 or value < MIN_INT32:
        raise OverflowError(
            "Fixed point Q%d.%d overflow" % (31 - frac_bits, frac_bits)
        )
    return value
...
class DigitalFilter:
    ...
    def add_notch(self, notch_freq, notch_quality):
        ...
        b, a = signal.iirnotch(
            notch_freq, Q=notch_quality, fs=self.sample_frequency
        )
    ...
    def add_derivative(self):
        ...
        self.filter_sections.append((1.0, -1.0, 0.0, 1.0, 0.0, 0.0))
    ...
    def get_initial_state(self):
        if self.initial_state is None:
            return [[0.0, 0.0]] * len(self.filter_sections)
...
class MCU_SosFilter:
    ...
    def _build_config(self):
        ...
        self._set_offset_scale_cmd = self._mcu.lookup_command(
            ...
            " scale=%i scale_frac_bits=%c auto_offset=%c",
            cq=self._cmd_queue,
        )
    ...
    def set_offset_scale(self, offset=0, scale=1.0, auto_offset=False):
        self._offset = offset
        self._scale = scale
    ...
    def reset_filter(self):
        ...
        if len(sos_state) != num_sections:
            raise ValueError(
                ...
                "sections (%i) must be equal" % (num_sections, len(sos_state))
            )
        ...
        if self._max_sections:
            self._set_active_cmd.send(
                [self._oid, num_sections, self._coeff_frac_bits]
            )
...
class MCU_trigger_analog:
    ...
    def home_start(
        ...
        sensor_update = 1.0 / self._sensor.get_samples_per_second()
    ...
    def home_wait(self, home_end_time):
        ...
        if res >= mcu.MCU_trsync.REASON_COMMS_TIMEOUT:
            ...
            raise self._printer.command_error(
                "Trigger analog error: %s" % (error_msg,)
            )

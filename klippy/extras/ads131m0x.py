...
class ADS131M0X:
    def __init__(self, config, sensor_type, num_channels):
        ...
        self.channel = config.getint(
            "adc_channel", 0, minval=0, maxval=num_channels - 1
        )
        ...
        if pwm_clock_name is not None:
            ...
            self.clock_freq = pwm_config.getfloat(
                "frequency",
                None,
                note_valid=False,
        ...
        if self.enable_global_chop:
            self.gc_dly = config.getchoice(
                "global_chop_delay", list(GC_DLY_TO_REG.keys()), 16
            )
        else:
            self.gc_dly = None

        # Sample rate setting
        config_sample_rate = config.getfloat(
            "sample_rate", above=0.0, default=500.0
        )
        ...
        for osr in OSR_TO_REG:
            ...
            if best_sps is None or abs(sps - config_sample_rate) < abs(
                best_sps - config_sample_rate
            ):
        ...
        logging.info(
            "%s '%s' configured sample_rate = %.1f SPS",
            self.sensor_type,
            self.name,
            best_sps,
        )
        ...
        self.spi = bus.MCU_SPI_from_config(
            config, mode=1, default_speed=4000000
        )
        ...
        if drdy_pin_mcu != self.mcu:
            raise config.error(
                ...
                " data_ready_pin must be on the same MCU" % (self.sensor_type,)
            )
    ...
    def _build_config(self):
        ...
        self.ffreader.setup_query_command(
            "query_ads131m0x_status oid=%c", oid=self.oid, cq=cmdqueue
        )
    ...
    def lookup_sensor_error(self, error_code):
        return self._sensor_errors.get(
            error_code, "Unknown %s error" % (self.sensor_type,)
        )
    ...
    def _calc_sample_rate(self, osr):
        if self.enable_global_chop:
            return self.clock_freq / (2.0 * (self.gc_dly + 3.0 * osr))
        else:
            return self.clock_freq / (2.0 * osr)
    ...
    def _convert_samples(self, samples):
        ...
        for ptime, val in samples:
            ...
            if top_byte != 0x00 and top_byte != 0xFF:
                ...
                logging.error(
                    "'%s' sample error: %s"
                    % (self.name, self.lookup_sensor_error(top_byte))
                )
                continue
    ...
    def setup_chip(self):
        ...
        if (
            not is_batch_mode
            and (actual_status_val & STATUS_REG_MASK) != WORD24_MODE
        ):
            raise self.printer.command_error(

...
extruder pressure-advance smoother kernels are now generated by a more
accurate fitting procedure, so extruder motion under input shaping (with
pressure advance) shifts slightly for all shaper types. No configuration
change is required, but printed results may differ marginally from before.
